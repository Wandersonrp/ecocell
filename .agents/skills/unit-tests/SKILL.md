---
name: unit-tests
description: Gera testes de unidade para um slice/feature do EcoCell seguindo TDD (Red→Green→Refactor), stack xUnit+Shouldly+Moq+Bogus e as regras normativas de `.Codex/rules/unit-tests-rules.md`. Use quando o usuário pedir para escrever, criar ou adicionar testes a uma feature específica.
---

# Workflow: escrever testes de unidade

> Regras normativas (stack, proibições, nomenclatura) estão em `.Codex/rules/unit-tests-rules.md` e têm precedência sobre qualquer decisão pontual. Este arquivo define **como executar** o processo.
